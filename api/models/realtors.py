...
class Realtors(User):
    ...
    bank_name = models.CharField(max_length=200, choices=BankNameChoices, blank=True, null=True)
    ...
    bank_type = models.CharField(max_length=200, choices=[('savings', 'Savings'), ('current', "Current")], default="savings")
        ...
        for child in descendants:
            ...
            result.append(
                RealtorReferralSerializer2(
                    id=child.id,
                    realtor=child.realtor,
                    sponsor=[
                        ancestor.realtor 
                        for ancestor in  child.get_ancestors().order_by("-level")[:3]
                    ]
                )     
            )
            # child.get_ancestors().order_by("-level")[:3] == up-line descendant
        return result
...
class Referrals(MPTTModel, CustomBaseModel):
    ...
    realtor = models.ForeignKey(Realtors, on_delete=models.CASCADE, related_name="referrals")
    ...
    def _downlines(self, generations=3):
        ...
        level__lte=self.level + generations
    ) 
...
class Commission(CustomBaseModel):
    ...
    realtor = models.ForeignKey(Realtors, on_delete=models.CASCADE, related_name="commissions")
